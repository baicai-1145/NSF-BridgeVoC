...
class Specs(Dataset):
    def __init__(self, 
        ...
        if dataset_name.lower() == "libritts":  # LibriTTS benchmark reading format
            self.wav_files = []
            lines = open(data_dir, 'r').readlines()
            for l in lines:
                cur_filename = l.strip().split('|')[0]
                self.wav_files.append(os.path.join(raw_wavfile_path, f"{cur_filename}.wav"))
            if subset == 'train':
                # shuffle the training files
                random.seed(3407)
                random.shuffle(self.wav_files)
        elif dataset_name.lower() == "ljs":  # LJS benchmark reading format
            self.wav_files = []
            lines = open(data_dir, 'r').readlines()
            for l in lines:
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_set = Specs(dataset_name=self.dataset_name,
                                   raw_wavfile_path=self.raw_wavfile_path,
            ...
            self.valid_set = Specs(dataset_name=self.dataset_name,
                                   raw_wavfile_path=self.raw_wavfile_path,
