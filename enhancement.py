...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default=None,
                        help="Path to YAML config file for inference; CLI args override values from config.")
    ...
    parser.add_argument("--sampling_type", type=str, default="sde_first_order", 
    ...
    if post_str in ['.txt', '.scp']:
        filelist = []
        lines = open(args.test_dir, 'r', encoding="utf-8").readlines()
        for l in lines:
            cur = l.strip()
            # 兼容 LibriTTS/LJS 这类带文本的清单：取 '|' 前面的路径
            if '|' in cur:
                cur = cur.split('|')[0]
            # 若没有扩展名，则默认补 .wav
            if not os.path.splitext(cur)[1]:
                cur = cur + ".wav"
            filelist.append(os.path.join(args.raw_wav_path, cur))
    else:  # dir
